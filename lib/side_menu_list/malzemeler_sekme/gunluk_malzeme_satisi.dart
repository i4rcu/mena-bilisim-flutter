...
class GunlukMalzemeSatisiListPage extends StatefulWidget {
  @override
  _GunlukMalzemeSatisiListPageState createState() =>
      _GunlukMalzemeSatisiListPageState();
}
class _GunlukMalzemeSatisiListPageState extends State<GunlukMalzemeSatisiListPage> {
  String _searchQuery = '';
  ...
  void initState() {
    ...
    BlocProvider.of<MalzemelerBloc>(context).add(
      fetchGunlukMalzemeSatisi("tablePrefix", "tableSuffix", "0"),
    );
  }
  ...
  onPressed: () {
    ...
    if (state is GunlukMalzemeSatisiLoaded) {
      _exportToExcelWithStyles(state.GunlukMalzemeSatisii);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          ...
                          if (pickedDate != null && pickedDate != _selectedDate) {
                            ...
                            BlocProvider.of<MalzemelerBloc>(context).add(
                              fetchGunlukMalzemeSatisi("tablePrefix", "tableSuffix", daysDifference.toString()),
                            );
                          }
                        },
                        child: Text(
        ...
        body: BlocBuilder<MalzemelerBloc, MalzemelerState>(
          builder: (context, state) {
            ...
            } else if (state is GunlukMalzemeSatisiLoading) {
              return Center(child: CircularProgressIndicator());
            } else if (state is GunlukMalzemeSatisiLoaded) {
              List<GunlukMalzemeSatisi> filteredCariHesaplar;
      
              if (_searchQuery.isEmpty) {
                filteredCariHesaplar = state.GunlukMalzemeSatisii;
              } else {
                filteredCariHesaplar = state.GunlukMalzemeSatisii.where((cariHesap) {
    ...
    void _exportToExcelWithStyles(List<GunlukMalzemeSatisi> faturalar) async {
  var excel = Excel.createExcel();

  // Rename the default sheet
  String defaultSheet = excel.getDefaultSheet()!;
  excel.rename(defaultSheet, 'Günlük Malzeme Satışı');

  Sheet? sheetObject = excel['Günlük Malzeme Satışı'];
  ...
  var filePath = '${directory.path}/Gunluk_Malzeme_Satisi.xlsx';
