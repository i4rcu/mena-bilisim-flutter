...
class BankaDetayPage extends StatefulWidget {
  final int logicalref;

  BankaDetayPage({required this.logicalref});

  @override
  _BankaDetayPageState createState() => _BankaDetayPageState();
}

class _BankaDetayPageState extends State<BankaDetayPage> {
          ...
          child: Scaffold(
            ...
            body: BlocProvider(
              create: (context) => BankalarBloc(ApiHandler())
                ..add(FetchBankaDetaylari(widget.logicalref)),
              child: BlocListener<BankalarBloc, BankalarState>(
                listener: (context, state) {
                  if (state is BankaDetaylarLoaded) {
                    if (_filterOptions.isEmpty) {
                      setState(() {
                        _filterOptions = state.BankaDetaylar
                    ...
                    Expanded(
                      child: BlocBuilder<BankalarBloc, BankalarState>(
                        builder: (context, state) {
                          if (state is BankaDetaylarLoading) {
